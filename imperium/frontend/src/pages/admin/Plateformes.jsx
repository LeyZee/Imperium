  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-or">Plateformes</h1>
            ...
            <tbody>
              {plateformes.map((p, i) => (
                <tr key={p.id} className={i % 2 === 0 ? 'bg-white/5' : ''}>
                  ...
                  <td className="py-3 px-4"><span className="badge">{p.devise}</span></td>
                  <td className="py-3 px-4"><button onClick={() => openEdit(p)} className="text-or hover:text-yellow-300"><Edit size={16} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="card w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold text-or">{editId ? 'Modifier' : 'Ajouter'} une plateforme</h2>
